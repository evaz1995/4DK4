...
packet_arrival_event(Simulation_Run_Ptr simulation_run, void * ptr)
{
  ...
  if(server_state(data->link.server1) == FREE)
	  start_transmission_on_link(simulation_run, new_packet, data->link.server1);
  else if (data->link.server2 != NULL && server_state(data->link.server2) == FREE)
	  start_transmission_on_link(simulation_run, new_packet, data->link.server2);
  else
