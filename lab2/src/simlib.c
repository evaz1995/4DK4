...
simulation_run_execute_event(Simulation_Run_Ptr simulation_run)
{
  ...
  xfree(current_container,sizeof(Event_Container));
}
...
simulation_run_free_memory(Simulation_Run_Ptr this_simulation_run)
{
  ...
  while (event_list->size > 0) {
    xfree((void*) simulation_run_get_event(this_simulation_run),sizeof(Event_Container));
  }

  /* Clean up the simulation_run. */
  xfree(this_simulation_run->eventlist, sizeof(Event_Container));
  xfree(this_simulation_run->clock, sizeof(Clock));
  xfree(this_simulation_run, sizeof(Simulation_Run));
}
